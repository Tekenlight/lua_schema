...
function __ENTITY_handler_class:is_valid(s)
	if((s ~= nil) and (type(s) ~= "string")) then
		error_handler.raise_validation_error(-1,
						"Field: {"..error_handler.get_fieldpath().."} is not a valid ENTITY", debug.getinfo(1));
		return false
	end
...
function __ENTITY_handler_class:to_xmlua(ns, s)
...
function __ENTITY_handler_class:to_schema_type(ns, s)
...
function __ENTITY_handler_class:to_cjson_struct(ns, s)
	if (false == self:is_valid(s)) then
		local msv = error_handler.reset_init();
		error(msv.status.error_message);
	end
...
function __ENTITY_handler_class:get_facets()
	local facets;
	if (self.facets == nil) then
		facets = {};
	else
...
function __ENTITY_handler_class:to_type(ns, i)
	if ('string' ~= type(i)) then error("Field: {"..error_handler.get_fieldpath().."} Input not a valid ENTITY"); end
...
function __ENTITY_handler_class:new(i)
	return self:to_type(nil, i);
end

local mt = { __index = __ENTITY_handler_class; } ;
local _factory = {};
