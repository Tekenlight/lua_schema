...
__short_handler_class.datatype = 'int';
...
function __short_handler_class:is_deserialized_valid(x)
	...
	if (not status) then
		...
		error_handler.raise_validation_error(-1,
						"Field:["..tostring(x).."]:{"..error_handler.get_fieldpath().."} is not a valid short", debug.getinfo(1));
		return false;
	end
	...
	if (1 ~= self.c_short_str_pattern:check(f)) then
		error_handler.raise_validation_error(-1,
					"Value of the field {"..error_handler.get_fieldpath().."}: "
						..tostring(f)..", is not in the lexical spcae of xsd:short", debug.getinfo(1));
		local msv = error_handler.reset_init();
		error(msv.status.error_message);
	end
...
function __short_handler_class:new(i)
