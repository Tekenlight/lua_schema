...
function __unsignedLong_handler_class:is_deserialized_valid(x)
	...
	if (f == nil) then
		error_handler.raise_validation_error(-1,
						"Field:["..tostring(x).."]:{"..error_handler.get_fieldpath().."} is not a valid unsignedLong", debug.getinfo(1));
		return false;
	end
	...
	if (1 ~= self.c_unsignedLong_str_pattern:check(f)) then
		error_handler.raise_validation_error(-1,
					"Value of the field {"..error_handler.get_fieldpath().."}: "
						..tostring(f)..", is not in the lexical spcae of xsd:unsignedLong", debug.getinfo(1));
		local msv = error_handler.reset_init();
		error(msv.status.error_message);
	end
	local sn = string.gsub(f, "([%+]?)(%d+)", "%2");
	sn = string.gsub(sn, "([%d]+)([%.]0+)", "%1");
	if (not nu.val_uint64_str(sn)) then
		error_handler.raise_validation_error(-1,
						"Field:["..tostring(f).."]:{"..error_handler.get_fieldpath().."} is not a valid string representation of unsignedLong", debug.getinfo(1));
		local msv = error_handler.reset_init();
		error(msv.status.error_message);
	end
	local n = ffi.new("uint64_t", ffi.C.strtoul(sn, ffi.C.NULL, 0));
...
function __unsignedLong_handler_class:new(i)
