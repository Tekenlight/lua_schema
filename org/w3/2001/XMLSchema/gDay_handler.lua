...
local __gDay_handler_class = {}

__gDay_handler_class.type_id = xml_gDay_utils.value_type.XML_SCHEMAS_GDAY;
__gDay_handler_class.type_name = 'gDay';
__gDay_handler_class.datatype = 'datetime';

function __gDay_handler_class:is_deserialized_valid(x)
	local s = tostring(x);
	return self:is_valid(s);
end

function __gDay_handler_class:is_valid(s)
	if((s ~= nil) and (type(s) ~= "cdata")) then
		error_handler.raise_validation_error(-1,
						"Field: {"..error_handler.get_fieldpath().."} is not a valid gDay", debug.getinfo(1));
		return false
	end
...
function __gDay_handler_class:to_xmlua(ns, s)
...
function __gDay_handler_class:to_schema_type(ns, s)
...
function __gDay_handler_class:to_cjson_struct(ns, s)
	if (false == self:is_valid(s)) then
		local msv = error_handler.reset_init();
		error_handler.raise_fatal_error(-1, msv.status.error_message, debug.getinfo(1));
	end
...
function __gDay_handler_class:to_type(ns, i)
	if ('string' ~= type(i)) then
		error_handler.raise_fatal_error(-1,
			"Field: {"..error_handler.get_fieldpath().."} Input not a valid gDay", debug.getinfo(1));
	end
...
function __gDay_handler_class:new(i)
	return self:to_type(nil, i);
end

local mt = { __index = __gDay_handler_class } ;
local _factory = {};
