...
__non_positive_integer_handler_class.type_name = 'integer';
__non_positive_integer_handler_class.datatype = 'integer';
...
function __non_positive_integer_handler_class:is_deserialized_valid(x)
	...
	if (f == nil) then
		error_handler.raise_validation_error(-1,
						"Field:["..x.."]:{"..error_handler.get_fieldpath().."} is not a valid nonPositiveInteger", debug.getinfo(1));
		return false;
	end
	...
	if (1 ~= self.c_non_positive_integer_str_pattern:check(f)) then
		error_handler.raise_validation_error(-1,
					"Value of the field {"..error_handler.get_fieldpath().."}: "
						..f..", is not in the lexical spcae of xsd:nonPositiveInteger", debug.getinfo(1));
		local msv = error_handler.reset_init();
		error(msv.status.error_message);
	end
...
function __non_positive_integer_handler_class:to_type(ns, f)
	...
	if (c_f == nil) then
		error_handler.raise_validation_error(-1,
						"Field:["..f.."]:{"..error_handler.get_fieldpath().."} is not a valid nonPositiveInteger", debug.getinfo(1));
		local msv = error_handler.reset_init();
		error(msv.status.error_message);
	end
