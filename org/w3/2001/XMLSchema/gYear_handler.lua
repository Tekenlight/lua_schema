...
local xml_gYear_utils = xmlua.XMLDateUtils.new();
local __gYear_handler_class = {}

__gYear_handler_class.type_id = xml_gYear_utils.value_type.XML_SCHEMAS_GYEAR
__gYear_handler_class.type_name = 'gYear';
__gYear_handler_class.datatype = 'datetime';

function __gYear_handler_class:is_deserialized_valid(x)
	local s = tostring(x);
...
function __gYear_handler_class:to_xmlua(ns, s)
...
function __gYear_handler_class:to_cjson_struct(ns, s)
	if (false == self:is_valid(s)) then
		local msv = error_handler.reset_init();
		error_handler.raise_fatal_error(-1, msv.status.error_message, debug.getinfo(1));
	end
...
function __gYear_handler_class:to_type(ns, i)
	if ('string' ~= type(i)) then
		error_handler.raise_fatal_error(-1,
			"Field: {"..error_handler.get_fieldpath().."} Input not a valid gYear", debug.getinfo(1));
	end
...
function __gYear_handler_class:new(i)
	return self:to_type(nil, i);
end

local mt = { __index = __gYear_handler_class; } ;
local _factory = {};
