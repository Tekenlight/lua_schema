...
local _factory = {};
eh_cache.add('{http://www.w3.org/2000/09/xmldsig#}SignatureProperties', _factory);



function _factory:new_instance_as_root()
    local type_handler = basic_stuff.get_element_handler('http://www.w3.org/2000/09/xmldsig#', 'SignaturePropertiesType');
    local obj =  type_handler:new_instance_as_global_element({
                                        ns = 'http://www.w3.org/2000/09/xmldsig#',
                                        local_name = 'SignatureProperties',
                                        generated_name = 'SignatureProperties',
                                        root_element = true,
                                        min_occurs = 1,
                                        max_occurs = 1});
    return obj;
