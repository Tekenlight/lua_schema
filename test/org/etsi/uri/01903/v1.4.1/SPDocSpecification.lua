...
local _factory = {};
eh_cache.add('{http://uri.etsi.org/01903/v1.4.1#}SPDocSpecification', _factory);



function _factory:new_instance_as_root()
    local type_handler = basic_stuff.get_element_handler('http://uri.etsi.org/01903/v1.3.2#', 'ObjectIdentifierType');
    local obj =  type_handler:new_instance_as_global_element({
                                        ns = 'http://uri.etsi.org/01903/v1.4.1#',
                                        local_name = 'SPDocSpecification',
                                        generated_name = 'SPDocSpecification',
                                        root_element = true,
                                        min_occurs = 1,
                                        max_occurs = 1});
    return obj;
