...
local _factory = {};
eh_cache.add('{http://uri.etsi.org/01903/v1.3.2#}SignedSignatureProperties', _factory);



function _factory:new_instance_as_root()
    local type_handler = basic_stuff.get_element_handler('http://uri.etsi.org/01903/v1.3.2#', 'SignedSignaturePropertiesType');
    local obj =  type_handler:new_instance_as_global_element({
                                        ns = 'http://uri.etsi.org/01903/v1.3.2#',
                                        local_name = 'SignedSignatureProperties',
                                        generated_name = 'SignedSignatureProperties',
                                        root_element = true,
                                        min_occurs = 1,
                                        max_occurs = 1});
    return obj;
