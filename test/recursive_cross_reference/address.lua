...
local _factory = {};
eh_cache.add('{}address', _factory);



function _factory:new_instance_as_root()
    local type_handler = basic_stuff.get_element_handler('', 'AddressType');
    local obj =  type_handler:new_instance_as_global_element({
                                        ns = '',
                                        local_name = 'address',
                                        generated_name = 'address',
                                        root_element = true,
                                        min_occurs = 1,
                                        max_occurs = 1});
    return obj;
...
function _factory:new_instance_as_ref(element_ref_properties)
    local type_handler = basic_stuff.get_element_handler('', 'AddressType');
    local obj = type_handler:new_instance_as_local_element({ ns = '',
                                               local_name = 'address',
