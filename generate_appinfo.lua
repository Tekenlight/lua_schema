...
local function generate_validation_routine(rule_set, module_name, code_output)
	...
	if (code_output[rule_set._attr.package] == nil) then
		...
		local code = [=[
local validate_map = require('service_utils.validate_map');
...
return ]=]..package_parts[n]..[=[;
	...
	if (]=] .. v.assertion._attr.condition..[=[) then
		validations_array[j] = {};
		validations_array[j].function = function(context, obj, addnl_obj)
	...
	validations_array[j].function = package[']=]..v.validation._attr.method..[=['];
