...
local get_type_q_name = function(elem)
    assert(elem.type_q_name ~= nil, "Unexpected condition where type q name is nil");
    return get_q_name(elem.type_q_name.ns, elem.type_q_name.local_name);
end
...
elem_code_generator.get_subelement_properties = function(model, dbg, meta)
    local _subelement_properties;

    if (meta ~= nil and meta.global_name ~= nil) then
        _subelement_properties = codegen_eh_cache.get(meta.global_name..':subelement_properties');
    end
    ...
    if (_subelement_properties == nil) then
        ...
        return _subelement_properties, true;
    end
        ...
        if (item.symbol_type == 'element' or item.symbol_type == 'any') then
            ...
            if (item.ref) then
                ...
                _subelement_properties[item.generated_q_name] =
                    elem_code_generator.get_element_handler(item.element, {to_generate_names = false, global = true});
            ...
            elseif (item.content_type == 'S') then
                if (ddebug) then print(debug.getinfo(1).source, debug.getinfo(1).currentline, item.element:get_name()); end
                _subelement_properties[item.generated_q_name] =
                    elem_code_generator.get_element_handler(item.element, {to_generate_names = false, global = false});
                _subelement_properties[item.generated_q_name].particle_properties.min_occurs = item.min_occurs;
                _subelement_properties[item.generated_q_name].particle_properties.max_occurs = item.max_occurs;
                _subelement_properties[item.generated_q_name].particle_properties.root_element = false;
                _subelement_properties[item.generated_q_name].particle_properties.generated_name = gn;
                ...
                if (item.explicit_type) then
                    if (item.element == nil) then
                        ...
                        assert(item.element ~= nil, "CAN THIS REALLY HAPPEN");
                    else
                        if (ddebug) then print(debug.getinfo(1).source, debug.getinfo(1).currentline, item.element:get_name()); end
                    end
    ...
    for i,v in ipairs(gse_backlog) do
        v.properties.generated_subelements = elem_code_generator.get_generated_subelements(v.properties)
    end
    if (ddebug) then print(debug.getinfo(1).source, debug.getinfo(1).currentline); end
    return _subelement_properties, false;
end
...
elem_code_generator.get_element_handler = function(elem, options)
    local to_generate_names = options.to_generate_names;
    if (to_generate_names == nil) then to_generate_names = false; end
    ...
    do
        ...
        if (content_type ~= 'S') then
            ...
            if (not from_cache) then props.generated_subelements = elem_code_generator.get_generated_subelements(props); end
...
elem_code_generator.gen_lua_schema = function(elem)
    ...
    local element_handler = elem_code_generator.get_element_handler(elem, {to_generate_names = true, global = true});
    local lns = {};
