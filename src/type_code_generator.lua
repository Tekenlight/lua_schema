...
type_code_generator.get_element_handler = function(typedef, options)
    ...
    do
        ...
        if (content_type ~= 'S') then
            ...
            props.subelement_properties, from_cache =
                elem_code_generator.get_subelement_properties(model, false, {caller = 'T',
                        global_name = 'T:'..get_schema_type_name(typedef) });
...
type_code_generator.gen_lua_schema_code_from_typedef = function(typedef, indent)
    ...
    local element_handler = type_code_generator.get_element_handler(typedef, {to_generate_names = true, global = true});
